...
export function generateInsights(campaigns: CampaignData[], currency: string): string[] {
  ...
  if (best && best.roas >= 3) {
    ...
    insights.push(`🚀 Scale "${cap(best.name)}" — ${best.roas.toFixed(1)}x ROAS is your top performer (${spendPct}% of spend). Increase budget 20–30% to capture more volume without disrupting the algorithm.`);
  }
  ...
  if (worst && worst.roas < 1.5 && worst.spend > totalSpend * 0.08) {
    ...
    insights.push(`⛔ Review "${cap(worst.name)}" — ${worst.roas.toFixed(1)}x ROAS is below break-even. ${note}`);
  }
  ...
  if (highCTRLowConv.length) {
    ...
    insights.push(`⚠️ ${names} ${highCTRLowConv.length > 1 ? 'have' : 'has'} strong CTR but zero conversions — the ad is getting clicks but the landing page or offer isn't converting. Check page load speed, form errors, and offer clarity.`);
  }
  ...
  if (dangerCampaigns.length) {
    insights.push(`🔁 Ad fatigue — ${dangerCampaigns.map(c => `"${cap(c.name)}" (${(c.frequency??0).toFixed(1)}x)`).join(', ')} exceeded frequency 4.0. Audiences are oversaturated. Pause and rotate creatives immediately.`);
  } else if (fatigueCampaigns.length) {
    insights.push(`🔁 Frequency warning on ${fatigueCampaigns.map(c => `"${cap(c.name)}"`).join(', ')} — frequency above 2.5 signals early fatigue. Start preparing fresh creatives now.`);
  }
  ...
  if (totalConversions > 0) {
    ...
    if (bestCPA && avgCPA > 0) {
      ...
      if (saving > avgCPA * 0.3) {
        insights.push(`📉 "${cap(bestCPA.name)}" has your lowest CPA at ${fmt(bestCPA.cpa,'currency',currency)} vs. account average of ${fmt(avgCPA,'currency',currency)}. Shifting budget toward this campaign could reduce cost per acquisition by ${fmt(saving,'currency',currency)}.`);
      }
    }
  }
  ...
  if (freqDanger.length) {
    insights.push(`🛑 Don't pause ${freqDanger.map(c => `"${cap(c.name)}"`).join(', ')} yet — frequency above 4.5 feels alarming but pausing resets the algorithm's learning phase. Instead: refresh the creative with a new angle, keep the same targeting and budget.`);
  }
  ...
  if (highCPMEarly.length && insights.length < 7) {
    insights.push(`⏳ ${highCPMEarly.map(c => `"${cap(c.name)}"`).join(', ')} ${highCPMEarly.length > 1 ? 'show' : 'shows'} high CPM but with limited spend — CPM fluctuates heavily in the first 48–72 hours. Wait until each campaign has at least $150 spend before making CPM-based decisions.`);
  }
  ...
  if (singleDayRisk.length && insights.length < 7) {
    insights.push(`⚠️ Don't make structural changes to ${singleDayRisk.map(c => `"${cap(c.name)}"`).join(', ')} based on this data. Under 5 conversions means the ROAS reading is statistically unreliable — wait for at least 10 conversions before pausing, scaling, or changing creative.`);
  }
