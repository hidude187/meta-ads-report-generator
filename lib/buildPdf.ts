// lib/buildPdf.ts
// Full PDF generation logic for MetriQuill Free.
// Called from ExportButtons (client-side only via dynamic import).

import { CampaignData, ClientInfo, KPISummary } from "./types";
import { fmt, BENCHMARKS, getCampaignBadge, generateRecommendations } from "./formatters";
import { hexToRgb, darkenHex } from "./colors";
...
export async function buildPdf(
  ...
  campaigns.forEach((c, i) => {
    ...
    doc.setFontSize(7);
    if (isArRow && hasAmiri) {
      AF(); doc.setTextColor(15, 23, 42); doc.text(rowName, tX[0] + 5, rowY); LF("normal");
    } else {
  ...
  doc.text("Upgrade to MetriQuill Pro", 26, recY + 10);
